...
const Cards = () => {
  ...
  return (
    <div className="grid grid-cols-1 z-0 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8 gap-y-16 place-items-center p-4">
      {dataTypes.map((item) => (
        <section key={item.id} className={`relative ${isDarkMode?"bg-[#272727]":"bg-white"} rounded-xl  p-4 shadow-lg max-w-[260px] w-full transform hover:scale-105 transition duration-300`}>
          ...
          <div className="flex flex-col items-start gap-4">
            <h1 className="text-2xl font-semibold w-full text-end ">{item.size} GB</h1>
